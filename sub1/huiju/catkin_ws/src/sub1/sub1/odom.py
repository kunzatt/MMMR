...
class odom(Node):
    ...
    def imu_callback(self,msg):
        ...
        if self.is_imu ==False :    
            self.is_imu=True
            # 튜플로 반환
            imu_q= Quaternion.euler_from_quaternion(msg.orientation.w, msg.orientation.x, msg.orientation.y, msg.orientation.z)
    ...
    def listener_callback(self, msg):
        ...
        if self.is_imu ==True:
            if self.is_status == False :
                ...
                '''
                self.x += linear_x * self.period * cos(self.theta)
                self.y += linear_x * self.period * sin(self.theta)
                ...
                '''
                q = Quaternion.from_euler(self.x, self.y, self.theta)
