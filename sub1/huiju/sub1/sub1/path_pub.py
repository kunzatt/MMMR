...
class pathPub(Node):
    ...
    def __init__(self):
        ...
        '''
        full_path="C:\Desktop\catkin_ws\src\ros2_smart_home\sub2\path\path_to_pub.txt"
        ...
        for line in lines :
            ...
            read_pose.pose.orientation.w=0.1
    ...
    def timer_callback(self):
        if self.is_odom ==True:
            ...
            for i,waypoint in enumerate(self.global_path_msg.poses) :
                way_x = waypoint.pose.position.x
                way_y = waypoint.pose.position.y

                distance=sqrt((way_x - x)**2 + (way_y - y)**2)
                if distance < min_dis :
                    min_dis=distance
                    current_waypoint=i
            ...
            if current_waypoint != -1 :
                if current_waypoint + self.local_path_size < len(self.global_path_msg.poses):                 
                    self.global_path_msg.poses.append(current_waypoint, current_waypoint + self.local_path_size)
                else :
                    self.global_path_msg.poses.append(current_waypoint, self.global_path)
