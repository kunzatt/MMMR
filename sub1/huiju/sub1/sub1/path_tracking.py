    ...
    def timer_callback(self):
        ...
        if self.is_status and self.is_odom ==True and self.is_path==True:
            ...
            if len(self.path_msg.poses)> 1:
                ...
                '''
                self.lfd= self.cmd_msg.linear.x
                ...
                for num,waypoint in enumerate(self.path_msg.poses) :
                    ...
                    self.current_point=num
                    way_x = waypoint.pose.position.x
                    way_y = waypoint.pose.position.y
                    dis=sqrt((way_x - robot_pose_x)**2 + (way_y - robot_pose_y)**2)
                ...
                if self.is_look_forward_point :
                    ...
                    (테스트) 맵에서 로봇의 위치(robot_pose_x,robot_pose_y)가 (5,5)이고, 헤딩(self.robot_yaw) 1.57 rad 일 때, 
                    선택한 전방포인트(global_forward_point)가 (3,7)일 때
                    ...
                    로직 7. 선속도, 각속도 정하기
                    '''             
                    out_vel=0.8/abs(180 - theta)
                    out_rad_vel=theta/self.time_period
