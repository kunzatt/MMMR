...
class Controller(Node):
    ...
    def timer_callback(self):
        ...
        '''
        로직1. 수신 데이터 출력
        터틀봇 상태 : 현재 선속속도, 현재 각속도, 배터리 상태, 충전 상태 출력
        환경 정보 : 날짜, 시간, 온도, 날씨 출력
        가전 제품 : 가전상태 출력        
        '''
        if self.is_turtlebot_status:
            print(f"터틀봇 상태: 선속도={self.turtlebot_status_msg.twist.linear.x:.2f}, " 
                f"각속도={self.turtlebot_status_msg.twist.angular.z:.2f}, "
                f"배터리={self.turtlebot_status_msg.battery_percentage:.1f}%, "
                f"충전상태={self.turtlebot_status_msg.power_supply_status}")
        
        if self.is_envir_status:
            print(f"환경 정보: 날짜={self.envir_status_msg.month}/{self.envir_status_msg.day}, "
                f"시간={self.envir_status_msg.hour:02d}:{self.envir_status_msg.minute:02d}, "
                f"온도={self.envir_status_msg.temperature:.1f}, "
                f"날씨={self.envir_status_msg.weather}")
