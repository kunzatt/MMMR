...
def transformMTX_lidar2cam(params_lidar, params_cam):
    ...
    Tmtx = translationMtx(*(cam_pos - lidar_pos))
    ...
    Rmtx = np.linalg.inv(R_cam) @ R_lidar
...
def project2img_mtx(params_cam):
    ...
    fov_rad = np.deg2rad(params_cam["FOV"])
    fc_x = params_cam["WIDTH"] / (2 * np.tan(fov_rad / 2))
    fc_y = params_cam["HEIGHT"] / (2 * np.tan(fov_rad / 2))
...
class LIDAR2CAMTransform:
    ...
    def transform_lidar2cam(self, xyz_p):
        ...
        xyz_p_homogeneous = np.hstack((xyz_p, ones))
        xyz_c = np.dot(xyz_p_homogeneous, self.RT.T)
...
class SensorCalib(Node):
    ...
    def timer_callback(self):
        ...
        if self.xyz is not None and self.img is not None :
            ...
            img_l2c = draw_pts_img(self.img.copy(), xy_i[:, 0], xy_i[:, 1])
